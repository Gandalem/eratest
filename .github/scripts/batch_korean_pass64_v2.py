...
def parse_tsv(raw: str) -> dict[str, str]:
    out: dict[str, str] = {}
    for row in raw.strip().splitlines():
        ...
        src, dst = row.split("\t", 1)
        if src in out:
            raise AssertionError(f"duplicate map key: {src}")
        out[src] = dst
    return out


TALENT_MAP = parse_tsv(r'''
Love	연모
...
SIMPLE_RHS_RE = re.compile(r"^(?P<prefix>\s*(?:RETURNF|LOCALS(?::\d+)?\s*(?:'=|\+=))\s*@?)\"(?P<value>(?:\\.|[^\"\\])*)\"(?P<suffix>\s*)$")
...
def read_text(path: Path) -> tuple[str, bool]:
    raw = path.read_bytes()
    return (raw.decode("utf-8-sig") if raw.startswith(BOM) else raw.decode("utf-8"), raw.startswith(BOM))
...
def checked(src: str, dst: str) -> str:
    a = TOKEN_RE.findall(src)
    b = TOKEN_RE.findall(dst)
    if a != b:
        raise AssertionError((src, dst, a, b))
    return dst
...
def apply_simple_rhs(root: Path, rel: str, mapping: dict[str, str]) -> int:
    ...
    for line in text.splitlines(keepends=True):
        ...
        m = SIMPLE_RHS_RE.match(body)
        if m and m.group("value") in mapping:
            src = m.group("value")
            body = f'{m.group("prefix")}"{checked(src, mapping[src])}"{m.group("suffix")}'
            count += 1
        out.append(body + nl)
    if count:
...
def apply_spell(root: Path) -> int:
    ...
    for line in text.splitlines(keepends=True):
        ...
        if m and m.group("value") in SPELL_MAP:
            src = m.group("value")
            line = line[:m.start("value")] + checked(src, SPELL_MAP[src]) + line[m.end("value"):]
            count += 1
        out.append(line)
    if count:
        write_text(path, "".join(out), bom)
...
def apply_trlib(root: Path) -> int:
    ...
    for line in text.splitlines(keepends=True):
        ...
        if line.lstrip().startswith(";"):
            out.append(line)
            continue
        if fn in {"UNIQUE_COM_TR", "NAME_FIELD_TYPE_TR", "SUIKA_WEAPON_TR"} and re.match(r"^\s*(?:RETURNF|LOCALS\s*(?:'=|\+=))", line):
            def repl(m: re.Match[str]) -> str:
                ...
                if src not in TRLIB_QUOTED_MAP:
                    return m.group(0)
                count += 1
                return '"' + checked(src, TRLIB_QUOTED_MAP[src]) + '"'
            line = QUOTE_RE.sub(repl, line)
        if fn in {"JOIN_IN_TR", "JOIN_IN_SINGLE_TR"}:
            nl = "\n" if line.endswith("\n") else ""
            body = line[:-1] if nl else line
            ...
            if bm:
                ...
                if src in TRLIB_BARE_MAP:
                    line = bm.group("prefix") + TRLIB_BARE_MAP[src] + bm.group("suffix") + nl
                    count += 1
        out.append(line)
    if count:
        write_text(path, "".join(out), bom)
...
def replace_padded(value: str) -> tuple[str, bool]:
    left = len(value) - len(value.lstrip(" "))
    right = len(value) - len(value.rstrip(" "))
    end = len(value) - right if right else len(value)
    core = value[left:end]
    if core not in LIST_MAP:
        return value, False
    return " " * left + checked(core, LIST_MAP[core]) + " " * right, True
...
def apply_list(root: Path) -> int:
    ...
    for line in text.splitlines(keepends=True):
        ...
        if stripped.startswith(";"):
            out.append(line)
            continue
        if stripped.startswith("#DIMS CONST DISP_NAME ="):
            def arr_repl(m: re.Match[str]) -> str:
                nonlocal count
                dst, changed = replace_padded(m.group(1))
                if changed:
                    count += 1
                    return '"' + dst + '"'
                return m.group(0)
            ...
            if lm and lm.group("value") in LIST_MAP:
                src = lm.group("value")
                body = lm.group("prefix") + '"' + checked(src, LIST_MAP[src]) + '"' + lm.group("suffix")
                count += 1
